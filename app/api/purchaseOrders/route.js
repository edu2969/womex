...
export async function POST(req) {
    try {
        ...
        for(var i = 0; i < pocs.length; i++) {
            ...
            if(!requester) {
                return NextResponse.json({ message: `Error - Requester ${poc.requesterName} not found` }); 
            }
            var creator = await User.findOne({ name: poc.creatorName });
            if(!creator) {
                return NextResponse.json({ message: `Error - Creator ${poc.creatorName} not found` }); 
            }
